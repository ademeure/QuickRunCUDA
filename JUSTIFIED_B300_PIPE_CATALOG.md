...
## TLDR — current audit state (2026-04-23, +§20 retest 2026-04-24, +§21 retest 2026-04-24, +§C5 deep-dive 2026-04-24)

**Coverage:** 56 catalog sections audited, 0 still pending (D7+E7 added 2026-04-23 as 🟡 closed-as-preserved due to single-GPU rig state; B6 DRAM-write clock-dep added 2026-04-23 as ⚠ refined; §20 FMIN-penalty retest added 2026-04-24 as ❌ baseline-falsified; §21 tcgen05.mma "sustained-load throttle" retest added 2026-04-24 as ❌ cliff-DOES-NOT-REPRODUCE + I-cache-hypothesis-falsified; **§C5 FFMA2+UNPACK 16% friction retest added 2026-04-24 as ✅ CONFIRMED with refined mechanism — operand-reuse cache disturbance, not raw RF read-port pressure**)
...
| §C5 FFMA2+UNPACK 16% friction (Rule 2) | L478 | ✅ replicated | [C5_ffma2_unpack_regport_DEEP.md](justifications/C5_ffma2_unpack_regport_DEEP.md) — **catalog 1.67/1.95 differential REPLICATED** in v52mirror shape (F2FP+FFMA2 u=3.37 vs PRMT+FFMA2 u=3.98, same 1.18× ratio = 16% friction). **Mechanism is NOT raw RF read-port pressure** (PRMT has MORE reads/cy than F2FP yet faster). Real mechanism = **`.reuse` cache disturbance**: F2FP UNPACK invalidates the SMSP operand-reuse cache slot used by FFMA2's broadcast multiplier R0 → reuse rate drops 99/128 (PRMT) → 79/128 (F2FP) → ~38% of FFMA2 instructions pay an extra RF read. **Regime caveat**: asymmetry only manifests in clustered FFMA2-broadcast loops; interleaved 1:1 with private multiplicands shows zero asymmetry (V1=V2=2.74). LOP3 1:1 also slow (u=2.14, 22% loss) — same mechanism, only 50% reuse retention. |
