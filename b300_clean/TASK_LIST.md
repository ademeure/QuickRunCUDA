...
- [x] NVFP4 CuTeDSL: TDP THROTTLE discovered. Sustained boost = 1455 MHz / 1095 W (NOT 2032), invalidates prior short-bench MFU. True per-active-SM MFU = 79.2% (boost) / 88.4% (510MHz lock). Cluster (2,4) wins at boost (data reuse > more SMs); (2,1) wins at 510 (compute slow enough that DRAM keeps up). 256×256 tile universally best; powers-of-2 only. data=RANDOM verified. Commit `bf46464`.
- [x] Clock locking on B300 SXM6: -lgc IS authoritative (verified 510-1500 MHz scaling; FFMA 0.461/0.233/0.156/0.112ms at 510/1000/1500/boost = perfect inverse). Earlier "1942 MHz floor" was leftover background procs thrashing GPU 0 — once killed, GPU drops to 120 MHz idle, locks honored. Multi-GPU configuration does NOT couple clocks. -ac silently ignored (use -lgc). Memory updated.
- [x] **N-dependence of cuBLAS K-id speedup** (4 commits): SHAPE-CONDITIONAL — full 1.40-1.55× speedup ONLY at N ∈ {K/2, K, 2K} for B-side K-id pattern; razor-sharp boundary (off by 32 = total loss); ncu confirms IDENTICAL cycles (pure clock throttle); kernel-universal (128×256 + 256×256 same rule); precision-universal (BF16 + FP8); A K-id gives ~0.5% (asymmetric); transB=1 KILLS speedup (memory-access mechanism). Op lesson: GPU stuck at 1005 MHz with NO explicit lock — required `-rgc` to fix. Commits `702d4bd` `3f98880` `4bdb423` plus 2 more. See `b300_clean/N_DEPENDENCE_DEEPDIVE.md`.
