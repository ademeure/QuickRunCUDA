...
#define MMA_K 96
// mode (u1):
//  0 = B in {+0..+2.0} (5 codes, no signs)         [no sign toggle]
//  1 = B in {+0..+2.0, -0..-2.0} (10 codes, ±)     [+sign toggle on 5 mags]
//  2 = B in 8 codes (low 3 bits) random, sign always +
//  3 = B in 16 codes (full random)
//  4 = B in {+0.5, +1.0, +1.5, +2.0} (4 NONZERO codes)
//  5 = same as 4 but with +/- (8 codes)
    ...
    if (threadIdx.x < 32) {
        for (int idx = threadIdx.x; idx < smem_size; idx += 32) {
            ...
            for (int p = 0; p < 8; p++) {
                ...
                unsigned fp4;
                switch (mode) {
                    case 0: fp4 = byte % 5u; break;        // 5 positive: 0..4
                    case 1: {                              // 10 codes: ±{0..2.0}
                        unsigned m = byte % 5u; unsigned s = (byte >> 4) & 1;
                        fp4 = (s << 3) | m; break;
                    }
                    case 2: fp4 = byte & 0x7u; break;      // 8 positives (full mantissa)
                    case 3: fp4 = byte & 0xFu; break;      // 16 codes (full random)
                    case 4: fp4 = (byte % 4u) + 1u; break; // 4 NONZERO positive: 1..4
                    case 5: {                              // 8 codes: ±{0.5..2.0}
                        unsigned m = (byte % 4u) + 1u; unsigned s = (byte >> 4) & 1;
                        fp4 = (s << 3) | m; break;
                    }
                    default: fp4 = 0;
                }
