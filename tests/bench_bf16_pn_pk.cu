// BF16 N-direction period p_n combined with K-direction flip period pk
// Pattern: sign[k][n] = ((n / p_n) & 1) XOR ((k / pk) & 1)
// Args: -1 p_n (N period), -2 pk (K flip period; 0 or >=K = no flip)
...
void kernel(float* A, float* B, float* C, int iters, int p_n, int pk) {
    ...
    if (threadIdx.x < 32) {
        for (int idx = threadIdx.x; idx < total_b_words; idx += 32) {
            ...
            int s0 = ((n0 / p_n) & 1) ^ kflip;
            int s1 = ((n1 / p_n) & 1) ^ kflip;
    ...
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        ...
        printf("BF16 p_n=%d pk=%d cy/MMA=%.2f\n", p_n, pk, (double)(t1-t0)/iters);
    }
}
