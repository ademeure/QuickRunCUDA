// BF16 random shift per K row: tests delta-predictor vs 2-cache hypothesis
// Each K row has a random shift in [0, p_n)
// Pattern: sign[k][n] = ((n + rand_shift[k]) / p_n) & 1
...
void kernel(float* A, float* B, float* C, int iters, int p_n, int u2) {
    ...
    if (threadIdx.x < 32) {
        for (int idx = threadIdx.x; idx < total_b_words; idx += 32) {
            ...
            // Random shift per K row
            unsigned ksr = (k * 0x9E3779B1u) ^ 0xC0FFEE13u;
            ksr ^= ksr >> 16; ksr *= 0xCAFEBABEu;
            int rand_shift = ksr % p_n;
            int s0 = ((n0 + rand_shift) / p_n) & 1;
            int s1 = ((n1 + rand_shift) / p_n) & 1;
    ...
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        ...
        printf("BF16 rand-shift-per-K p_n=%d cy/MMA=%.2f\n", p_n, (double)(t1-t0)/iters);
    }
}
