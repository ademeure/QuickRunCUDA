// BF16 +-+-+- in N direction, with K-direction periodic flip every pk rows
// Pattern: sign[k][n] = (n & 1) XOR ((k / pk) & 1)
// Args: -1 pk (use 0 or 999 for infinite = no flip)
...
void kernel(float* A, float* B, float* C, int iters, int pk, int u2) {
    ...
    int npacks = MMA_N / 2;  // 128 packs per K row (BF16, 2 per word)
    ...
    if (threadIdx.x < 32) {
        for (int idx = threadIdx.x; idx < total_b_words; idx += 32) {
            ...
            // pk=0 or very large means no flip (infinite period)
    ...
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        ...
        printf("BF16 alt-kflip M=%d N=%d K=%d pk=%d cy/MMA=%.2f\n",
               MMA_M, MMA_N, MMA_K, pk, (double)(t1-t0)/iters);
    }
}
