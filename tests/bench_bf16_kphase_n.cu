// BF16 K-phase with LIMITED number of distinct phases
// Tests "cache holds N patterns" hypothesis
// Args: -1 period_k, -2 num_phases (cap on distinct phases per column)
...
void kernel(float* A, float* B, float* C, int iters, int period_k, int num_phases) {
    ...
    if (threadIdx.x < 32) {
        for (int idx = threadIdx.x; idx < total_b_words; idx += 32) {
            ...
            // Random phase per N column from [0, num_phases) - LIMITED to num_phases distinct values
            auto phase_for_n = [num_phases, period_k](int n) -> int {
                unsigned pr = (n * 0x9E3779B1u) ^ 0xC0FFEEABu;
                pr ^= pr >> 16; pr *= 0xCAFEBABEu;
                int phase_bucket = pr % num_phases;
                // Spread the buckets evenly across [0, period_k)
                return (phase_bucket * period_k) / num_phases;
            };
    ...
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        ...
        printf("BF16 K-phase-N M=%d N=%d K=%d period_k=%d num_phases=%d cy/MMA=%.2f\n",
               MMA_M, MMA_N, MMA_K, period_k, num_phases, (double)(t1-t0)/iters);
    }
}
