// BF16 tcgen05 data-dependent power test (laneid early-exit)
#define MMA_M 256
#define MMA_N 256
#define MMA_K 16
...
void kernel(float* A, float* B, float* C, int iters, int mode, int u2) {
    // mode: 0=random A+B, 1=random A, B=0, 2=A=0, B=random,
    //       3=both zero, 4=both positive, 5=B small positive
    unsigned laneid;
    asm volatile("mov.u32 %0, %%laneid;" : "=r"(laneid));
    if (laneid != 0) return;

    extern __shared__ unsigned smem[];
    int a_size = 2048;   // 256*16/2
    int b_size = 2048;   // 16*256/2
    ...
    for (int i = 0; i < a_size; i++) {
        ...
        r &= ~0x40004000u; r |= 0x3E003E00u;  // finite values
        unsigned val = r;
        if (mode == 2 || mode == 3) val = 0;                     // A=0
        else if (mode == 4) val = r & ~0x80008000u;             // A positive
        smem_A[i] = val;
    }
    for (int i = 0; i < b_size; i++) {
        unsigned r = (i + blockIdx.x * 1024u + 0xC0FFEE00u) * 0x9E3779B1u;
        r ^= r >> 16; r *= 0x85EBCA6Bu; r ^= r >> 13; r *= 0xC2B2AE35u; r ^= r >> 16;
        r &= ~0x40004000u; r |= 0x3E003E00u;
        unsigned val = r;
        if (mode == 1 || mode == 3) val = 0;                     // B=0
        else if (mode == 4) val = r & ~0x80008000u;             // B positive
        else if (mode == 5) val = (r & 0x00FF00FFu) | 0x3E003E00u;  // small positive
        smem_B[i] = val;
    }
