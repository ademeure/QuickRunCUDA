// NVFP4 K=96: laneid early-exit BEFORE alloc — only lane 0 does everything
#define MMA_M 256
#define MMA_K 96
#ifndef MMA_N
#define MMA_N 256
...
void kernel(float* A, float* B, float* C, int iters, int mode, int u2) {
    ...
    unsigned* smem_B = smem + (MMA_K * MMA_N / 8);
    unsigned long long* mbar_p = (unsigned long long*)(smem_B + (MMA_K * MMA_N / 8));
    unsigned* tmem_p = (unsigned*)(mbar_p + 1);

    int smem_size = MMA_K * MMA_N / 8;
    for (int idx = 0; idx < smem_size; idx++) {
    ...
    for (int idx = 0; idx < smem_size; idx++) {
        ...
        for (int p = 0; p < 8; p++) {
            ...
            unsigned fp4 = (mode == 10) ? (byte % 5u) : (mode == 0 ? (byte & 0xFu) : 0u);
            val |= ((fp4 & 0xFu) << (p * 4));
        }
        smem_B[idx] = val;
    }
    ...
    *tmem_p = 0xFFFFFFFFu;
    ...
    if ((blockIdx.x & 1) == 0) {
        ...
        if (blockIdx.x == 0) {
            ...
            printf("M=%d N=%d cy/MMA=%.2f\n", MMA_M, MMA_N, (double)(t1-t0)/iters);
        }
    }
    asm volatile("barrier.cluster.arrive.aligned;");
    asm volatile("barrier.cluster.wait.aligned;");
