// Test 1c: mul1 self-op (chain via mul1 slot only; mul2/addend distinct)
// fma.rn.f32 %0, %0, %1, %2
// Note: result feeds back through MUL slot, not ADD slot
#ifndef N_INNER
#define N_INNER 1024
#endif
#ifndef N_OUTER
...
void kernel(float* A, float* B, float* C, int seed, int u1, int u2) {
    ...
    for (int i = 0; i < N_OUTER; i++) {
        ...
        for (int j = 0; j < N_INNER; j++) {
            // d = d * k1 + k2
            asm volatile("fma.rn.f32 %0, %0, %1, %2;" : "+f"(v) : "f"(k1), "f"(k2));
        }
    }
    ...
    if (blockIdx.x == 0 && threadIdx.x == 0) {
        ...
        printf("v1c_mul1_selfop total=%llu clk=%llu cy/op=%.4f\n",
