// D6 v2: scan number of independent FFMA chains to find RF saturation
// Each chain has a distinct accumulator + 2 distinct operand sources
#ifndef NCHAINS
#define NCHAINS 4
#endif
...
void kernel(float* A, float* B, float* C, int ITERS, int seed, int u2) {
    ...
    if (blockIdx.x == 0 && threadIdx.x == 0) {
        ...
        printf("NCHAINS=%d clk=%llu cy/iter=%.3f cy/fma=%.4f thru=%.3f fma/cy\n",
               NCHAINS, t1-t0, (double)(t1-t0)/(double)ITERS,
