// D6 v3: force all 3 FFMA operands to be REGISTERS (not immediate)
// to test if RF read port becomes the bottleneck
#ifndef NCHAINS
#define NCHAINS 16
#endif
...
void kernel(float* A, float* B, float* C, int ITERS, int seed, int u2) {
    ...
    float a[16];
    float ya[16];
    // za is computed at runtime from u2 to force register storage (defeat constant folding)
