// BF16 tcgen05 N-sweep with laneid early-exit (clean SASS)
#define MMA_M 256
#define MMA_K 16
#ifndef MMA_N
#define MMA_N 256
...
void kernel(float* A, float* B, float* C, int iters, int mode, int u2) {
    ...
    // BF16: 2 elements per unsigned. Sizes in dwords.
    int a_size = MMA_M * MMA_K / 2;      // 256*16/2 = 2048
    int b_size = MMA_K * MMA_N / 2;      // 16*N/2 = 8*N
    ...
    // Fill A: random finite BF16
    for (int i = 0; i < a_size; i++) {
        unsigned r = (i + blockIdx.x * 1024u) * 0x9E3779B1u;
        r ^= r >> 16; r *= 0x85EBCA6Bu; r ^= r >> 13; r *= 0xC2B2AE35u; r ^= r >> 16;
        // clear bit 14 to avoid huge exp; set bit 13 for moderate exp
        r &= ~0x40004000u;
        r |= 0x3E003E00u;
        smem_A[i] = r;
    }
    // Fill B: random finite BF16 (or controlled by mode)
    ...
    *tmem_p = 0xFFFFFFFFu;
    ...
    // idesc for BF16 (c=F32, a=BF16, b=BF16, no block_scale)
