...
void kernel(float* A, float* B, float* C, int iters, int sf_mode, int outliers_per_block) {
    ...
    if (threadIdx.x < 32) {
        for (int idx = threadIdx.x; idx < smem_size; idx += 32) {
            ...
            for (int p = 0; p < 8; p++) {
                int n = n_base + p;
                // Outlier decision: pick `outliers_per_block` random positions in 0..15
                // We hash (n, kblock) to derive a bitmask of "outlier positions"
                unsigned out_h = mix32((unsigned)n * 0x9E3779B1u + (unsigned)kblock * 0xCAFEBABEu);
                bool is_outlier = false;
                if (outliers_per_block > 0) {
                    // Use first outliers_per_block "winners" — mark them via Fisher-Yates-lite
                    unsigned char positions[16];
                    for (int q = 0; q < 16; q++) positions[q] = (unsigned char)q;
                    unsigned r2 = out_h;
                    for (int q = 15; q > 0; q--) {
                        r2 = r2 * 1103515245u + 12345u;
                        unsigned j = r2 % (q + 1);
                        unsigned char t = positions[q]; positions[q] = positions[j]; positions[j] = t;
                    }
                    for (int q = 0; q < outliers_per_block && q < 16; q++) {
                        if (positions[q] == k_in_block) { is_outlier = true; break; }
                    }
                }
    ...
    unsigned tsfb_addr = tmem_addr + 256;
    {
        for (int chunk = 0; chunk < 4; chunk++) {
            ...
            unsigned pa = (sf_mode == 0) ? 0x38383838u : mix32(chunk * 100u + threadIdx.x);
            unsigned pb = (sf_mode == 0) ? 0x38383838u : mix32(chunk * 200u + threadIdx.x + 1u);
            unsigned pc = (sf_mode == 0) ? 0x38383838u : mix32(chunk * 300u + threadIdx.x + 2u);
            unsigned pd = (sf_mode == 0) ? 0x38383838u : mix32(chunk * 400u + threadIdx.x + 3u);
