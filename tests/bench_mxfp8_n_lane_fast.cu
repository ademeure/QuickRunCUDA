// MXFP8 tcgen05 N-sweep, block_scale.block32, UE8M0 SF, laneid early-exit
#ifndef MMA_M
#define MMA_M 256
#endif
#define MMA_K 32
...
void kernel(float* A, float* B, float* C, int iters, int mode, int u2) {
    ...
    *tmem_p = 0xFFFFFFFFu;
    ...
    // SF = UE8M0 0x7F (=1.0) — MXFP8/MXFP4 use UE8M0
    { unsigned sf_pack = 0x7F7F7F7Fu;
      for (int ch=0; ch<4; ch++) {
        unsigned addr = tmem_addr + ch*128;
        asm volatile("tcgen05.st.sync.aligned.32x32b.x4.b32 [%0], {%1,%2,%3,%4};"
          :: "r"(addr),"r"(sf_pack),"r"(sf_pack),"r"(sf_pack),"r"(sf_pack));
      }
    }
    asm volatile("tcgen05.wait::st.sync.aligned;");

    // idesc: a=0, b=0 (FP8 E4M3), scale_format=1 (bit 23) for UE8M0
    ...
    if (true) {
        ...
        for (int i = 0; i < iters; i++) {
            asm volatile(
                ...
                "1"
#endif
                ".kind::mxf8f6f4.block_scale.block32 [%0], %1, %2, %3, [%5], [%6], P;\n\t}"
        ...
        if (blockIdx.x == 0) {
            ...
            printf("MXFP8 M=%d N=%d K=%d cta=%d cy/MMA=%.2f\n",
