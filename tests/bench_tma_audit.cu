// Audit: instrument TMA timings with actual poll count + data-dependency.
// If wait exits early, we'll see poll_count much less than expected OR data garbage.
// Writes per-CTA: {total_cyc, poll_count, data_sum}.
...
void kernel(float* A, float* B, float* C, int ITERS, int seed, int u2) {
    ...
    if (threadIdx.x == 0) {
        ...
        for (int i = 0; i < ITERS; i++) {
            // Stride stays within 1 GB buffer (caller MUST pass -A 268435456)
            unsigned long long off = ((unsigned long long)blockIdx.x * NTMAS * TMA_BYTES
                                    + (unsigned long long)i * STRIDE) & 0x3FFFFFFFull;
            ...
            for (int k = 0; k < NTMAS; k++) {
                asm volatile(
                    ...
                    " [%0], [%1], %3, [%2];"
                    :: "r"(smem_addr),  // all TMAs stomp same smem (stride=0)
                       "l"(src_base + k * TMA_BYTES),
                       "r"(mbar_addr),
                       "n"((unsigned)TMA_BYTES) : "memory");
            }

            // Instrumented wait: count polls and bail safely
            unsigned p = 0;
            unsigned polls = 0;
            for (int t = 0; t < 10000000 && !p; t++) {
                asm volatile(
                    "{ .reg .pred P; "
            ...
            // Force dependency on loaded data
            asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
            unsigned int x;
    ...
    __syncthreads();
    if (threadIdx.x == 0) {
        ((unsigned long long*)C)[blockIdx.x * 4 + 0] = t1 - t0;
        ((unsigned long long*)C)[blockIdx.x * 4 + 1] = total_polls;
        ((unsigned int*)C)[blockIdx.x * 8 + 4] = data_xor;
    }
}
