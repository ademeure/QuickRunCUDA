// Ninja B: relaxed atomic add + explicit fence
// Hypothesis: split the membar from the atomic. Use:
//   __threadfence();       // ensure prior writes globally visible BEFORE atomic
//   atomicAdd_relaxed (= atom.relaxed.gpu.global.add)
//   spin with relaxed loads
//   __threadfence();       // acquire-side after sync
//
// The acq_rel atomic is "atomic + acquire on its load + release on its store".
// We want to test if separating the fence (which is a single global membar) from
// the atomic data flow lets us avoid duplicating the membar overhead on every block.
//
// Key insight: the catalog notes atomic acq_rel is 1598 cy because it does an internal membar
// each time. If we use one membar before the atomic phase and rely on the L2 to provide
// causal ordering for the spin, we may save the per-block membar.
...
// Relaxed atomicAdd, relaxed spin loads. Single fence before the atomic and after the spin.
__device__ __forceinline__ void ninja_B_grid_sync(
    unsigned long long* arrival, int phase, int gridSz)
{
    __syncthreads();
    if (threadIdx.x == 0) {
        unsigned long long thresh = (unsigned long long)(phase + 1) * (unsigned long long)gridSz;
        // Release-side fence: ensure all prior writes are GPU-visible
        asm volatile("fence.release.gpu;" ::: "memory");
        // Relaxed atomic add - cheaper than acq_rel
        unsigned long long dummy;
        asm volatile("atom.relaxed.gpu.global.add.u64 %0, [%1], 1;"
                     : "=l"(dummy) : "l"(arrival) : "memory");
        // Spin with relaxed loads (avoid acquire fence per-iteration)
        while (true) {
            unsigned long long cur;
            asm volatile("ld.relaxed.gpu.global.u64 %0, [%1];" : "=l"(cur) : "l"(arrival) : "memory");
            if (cur >= thresh) break;
        }
        // Acquire-side fence: ensure subsequent loads see all writes from peers
...
void kernel_ninja_B(unsigned long long* out_cycles,
    ...
    for (int it = 0; it < ITERS; it++) {
        ...
        ninja_B_grid_sync(arrival, it, gridSz);
...
int main(int argc, char** argv) {
    ...
    kernel_ninja_B<<<grid, threads>>>(d_out_cy, d_out_pp, d_arrival, d_phase, iters);
    ...
    printf("# ninja_B: relaxed atomic add + manual fence(release/acquire)\n");
