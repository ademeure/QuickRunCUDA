...
void kernel(float* A, float* B, float* C, int iters, int mode, int u2) {
    ...
    unsigned* smem_B = smem + 2048;
    unsigned long long* mbar_p = (unsigned long long*)(smem_B + 2048);
    ...
    for (int i = 0; i < 2048; i++) {
        ...
        if (mode == 2 || mode == 3) r &= ~0x80000000u;
    ...
    for (int i = 0; i < 2048; i++) {
        ...
        r &= ~0x7F800000u; r |= 0x3F000000u;
        if (mode == 1 || mode == 3) r &= ~0x80000000u;  // TF32 sign
    ...
    *tmem_p = 0xFFFFFFFFu;
    ...
    unsigned idesc = (1U<<4)|(2U<<7)|(2U<<10)|(((unsigned)MMA_N>>3)<<17)|(((unsigned)MMA_M>>4)<<24);
