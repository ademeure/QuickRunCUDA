...
void kernel(float* A, float* B, float* C, int iters, int a_pm, int a_pk) {
    // Apply sign-period pattern to A along M-axis (pm) and K-axis (pk)
    // A operand has M × K shape = 256 × 96. Mag always +1.0.
    ...
    int npacks = MMA_N / 8;   // same layout as B (MxK), but M axis used
    if (threadIdx.x < 32) {
        for (int idx = threadIdx.x; idx < smem_size; idx += 32) {
            int k = idx / npacks;
            int mpack = idx % npacks;
            int m_base = mpack * 8;
            int kflip = (a_pk > 0) ? ((k / a_pk) & 1) : 0;
            unsigned val = 0;
            for (int p = 0; p < 8; p++) {
                int m = m_base + p;
                int mflip = (a_pm > 0) ? ((m / a_pm) & 1) : 0;
                int sign = kflip ^ mflip;
                unsigned fp4 = 0x2u | (sign << 3);
                val |= (fp4 << (p * 4));
            }
            smem_A[idx] = val;
        }
    ...
    if (threadIdx.x < 32) {
        // B = random
        for (int idx = threadIdx.x; idx < smem_size; idx += 32) {
            unsigned r = (idx + blockIdx.x * 1024u + 0xCAFE0000u) * 0x9E3779B1u;
