// NVFP4 K-period with per-column random phase
// sign[k][n] = ((k + phase[n]) / period_k) & 1
// FP4: K=64, N=256, 8 FP4 per word
...
void kernel(float* A, float* B, float* C, int iters, int period_k, int u2) {
    ...
    if (threadIdx.x < 32) {
        for (int idx = threadIdx.x; idx < smem_size; idx += 32) {
            ...
            for (int p = 0; p < 8; p++) {
                int n = n_base + p;
                // Random phase per column from [0, period_k)
                unsigned pr = (n * 0x9E3779B1u) ^ 0xC0FFEEABu;
                pr ^= pr >> 16; pr *= 0xCAFEBABEu;
                int phase = pr % period_k;
                unsigned fp4 = (r >> (p * 4)) & 0x7;
                int sign = ((k + phase) / period_k) & 1;
                fp4 |= (sign << 3);
                val |= (fp4 << (p * 4));
            }
            smem_B[idx] = val;
    ...
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        ...
        printf("NVFP4 K-phase M=%d N=%d K=%d period_k=%d cy/MMA=%.2f\n",
