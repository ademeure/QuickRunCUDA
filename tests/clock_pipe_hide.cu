// Directly test: does having FMAs AFTER the clock read hide its latency?
//
// VAR: split the FFMA work. B_BEFORE FMAs before clock read, B_AFTER FMAs after.
// Total FMAs fixed at B_BEFORE + B_AFTER. Sweep the split.
...
void kernel(float* A, float* B, float* C, int u0, int seed, int u2) {
    ...
    for (int i = 0; i < ITERS; i++) {
        // B_BEFORE FMAs (in 8-wide chains)
        #pragma unroll
        for (int k = 0; k < B_BEFORE/8; k++) {
        ...
        acc ^= c;
#endif
        // B_AFTER FMAs after clock read
        #pragma unroll
        for (int k = 0; k < B_AFTER/8; k++) {
            v0=v0*y+v0; v1=v1*y+v1; v2=v2*y+v2; v3=v3*y+v3;
            v4=v4*y+v4; v5=v5*y+v5; v6=v6*y+v6; v7=v7*y+v7;
        }
    }
