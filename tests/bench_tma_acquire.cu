// TMA prod/cons using mbarrier.try_wait.parity.acquire.cta — no explicit fence.
// Acquire/release semantics in mbarrier should replace fence.proxy.async.
...
void kernel(float* A, float* B, float* C, int ITERS, int seed, int u2) {
    ...
    if (threadIdx.x == 0) {
        ...
        for (int i = 0; i < ITERS; i++) {
            ...
            unsigned p = 0;
            // Blocking try_wait.parity.acquire (no explicit fence after — semantics carry)
            while (!p) {
                asm volatile(
                    "{ .reg .pred P; "
        ...
        for (int i = 0; i < ITERS; i++) {
            ...
            c_ph ^= (1u << slot);
            // No explicit fence — acquire in try_wait carries the ordering.
            unsigned int x;
