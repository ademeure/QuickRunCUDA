...
void kernel(float* A, float* B, float* C, int iters, int mode, int verify) {
    ...
    // N=64 K=16 BF16 = 1024 BF16 values = 512 unsigned words
    // Layout: idx = k * 32 + n_pair (n_pair=0..31, k=0..15)
    if (threadIdx.x < 32) {
        for (int idx = threadIdx.x; idx < 512; idx += 32) {
            ...
            if (mode == 200) {
                ...
                // PURE N-vary HIGH-ENTROPY for N=64: N_unique = 1<<(mode-2700)
                // N=64: 32 word_pairs per K row (idx = k * 32 + n_pair)
                int N_unique = 1 << (mode - 2700);
                int npair = idx % 32;   // CHANGED for N=64
                int n_even = npair * 2;
                int n_idx_e = n_even % N_unique;
                int n_idx_o = (n_even + 1) % N_unique;
    ...
    unsigned long long LBO = 16, SBO = 128;  // N=64: K stride = 32 words * 4 bytes = 128
