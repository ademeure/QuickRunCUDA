...
- [ ] **D6** "FP4 NVFP4 K=64 = 9856 TFLOPS" — ⚠ **DEMOTED 2026-04-23**: previously marked [x] based on catalog's own claims (L9298+), but I have NOT independently verified. Catalog claims `kind::mxf4nvf4.block_scale.block16` → 9.9 PFLOPS via `UTCOMMA.BLOCK16` SASS, with 15/15 correctness tests, AND that K=96 via idesc bit 31 doesn't add MACs. These are PLAUSIBLE catalog claims but not in this audit's verified set until I run the test myself. **NEW: agent dispatched to actually replicate (with full logging).** — `[ref: catalog L9213-L9430, agent in flight]`
