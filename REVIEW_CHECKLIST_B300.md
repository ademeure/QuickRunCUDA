...
- [x] **B6** "DRAM write = 7.09 TB/s" — ⚠ REFINED via §B6 (justifications/B6_dram_write_clock_dep.md): clock-locked sweep on GPU 0 measured DRAM write **5.51 TB/s @ 1500 / 6.20 @ 1700 / 6.75 @ 1920 / 6.86 @ 2032 MHz**. Catalog 7.09 is OPTIMISTIC by 3-30% depending on clock state. **User's hypothesis CONFIRMED**: SM→L2 write path scales linearly with SM clock (via `lts__t_sectors_srcunit_tex_op_write` cross-check) and is the binding regime ≤ 1700 MHz. Above 1700 MHz, HBM ceiling 7.18 TB/s starts to bind (94-96% MFU). Caveat: SM→L2 SoL is uniformly 74-80% across all clocks (never reaches the 32 B/clk × 148 SMs theoretical), so the practical SM-side cap is ~25 B/clk/SM not 32. **Canonical write peak = 6.86 TB/s @ 2032 MHz boost / 6.75 @ 1920 MHz lock.** Catalog should add explicit clock annotation. — `[ref: B300_PIPE_CATALOG.md:46, reviewed_errors L1132]`
