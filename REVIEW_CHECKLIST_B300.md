...
- [x] **D6** "FP4 NVFP4 K=64 = 9856 TFLOPS" — **RESOLVED 2026-04-23 (catalog itself, L9298+)**: real path is `kind::mxf4nvf4.block_scale.block16` (SASS `UTCOMMA.BLOCK16`); **measured 9.9 PFLOPS at K=64 = 99% of 10 PFLOPS spec**. Catalog has full correctness verification (15/15 tests). The **K=96 ULTRA via idesc bit 31 is FALSIFIED by the catalog itself** (L9349-L9360): K=96 doesn't compute additional MACs (output identical to K=64 with same A,B,scale). The proper `kind::mxf4` form is rejected by ptxas 13.2.78. — `[ref: catalog L9213-L9430]`
