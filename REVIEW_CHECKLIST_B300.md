...
- [ ] **CRIT11** ⚠ NEW 2026-04-23: catalog L1948 admits "self-op chains are 2× inflated from register read-port pressure". Many latency table entries (catalog L2017+ §24) use self-op chain methodology. The "true" architectural latencies may be ~2× lower than catalog says. Specifically suspect: FFMA = 4 cy (might be ~2 cy real), LDS = 33 (might be ~16-17 cy real), L1 = 43 (might be ~21-22 cy real), MUFU.EX2 = 14 (might be ~7 cy real), DFMA = 92/63.9/300 (multiple inconsistent values across catalog itself). The audit's JUSTIFIED §24 confirmed catalog numbers but didn't separately test distinct-chain methodology to falsify the inflation. — `[ref: catalog L1948, JUSTIFIED §24]` — `[methodology-narrow]`
