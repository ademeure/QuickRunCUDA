...
- [ ] **CRIT11** ⚠ NEW 2026-04-23: catalog L1948 admits "self-op chains are 2× inflated from register read-port pressure". **PER USER (2026-04-23): "we CANNOT rely on any claims that the 'real thing' is 2× without evidence in any individual case"** — agent dispatched to investigate per-operand-position, .reuse cache effect, RAW vs read-port mechanism, and which specific catalog latency entries are actually inflated. The "2×" claim itself is now itself a hypothesis to test, not a fact. — `[ref: catalog L1948, JUSTIFIED §24, justifications/SELF_OP_DEEP.md (in flight)]` — `[methodology-narrow]`
