...
- [ ] **G2b** OPEN: TMA vs LDG.E.128 max-tuned head-to-head — needed because catalog reports them in different sections with different methodologies, so cross-comparison is unreliable. The user-relevant question is: with both paths fully tuned (depth-pipelined / right tile size / full chip / anti-DCE), what's the gap (a) at L2-hit and (b) at DRAM-cold? Hypothesis: gap is small in both regimes (~1-3pp) but TMA may win L2-hit due to wider effective burst. Test in flight (added to the "next batch" agent dispatch list). — `[ref: feedback_tma_vs_ldg_comparison.md]` — `[regime-narrow]`
