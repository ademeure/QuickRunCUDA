...
- [ ] **C9** **Re-framing of dual-issue:** Scalar FFMA already saturates dispatch (3.88 of 4 = 97%) so cannot be usefully co-issued with anything else while staying fully utilized. ANY co-issue (LOP3, FFMA2, etc.) must displace FFMA dispatch slots. The interesting open question is **FFMA2 (packed, cap 2.00) + ALU op**: does that combination give MORE total useful ops/cy than scalar FFMA alone? Catalog L481 hints "Peak rate for mixed ~2.0 total warp-inst/SM/cy (one must yield)" but wave-7 NEEDS a clean test of FFMA2+LOP3 specifically, with both throughput AND useful-op accounting (FFMA2 = 2 ops/inst). — `[ref: catalog L481, project_b300_corrections_swarm.md V52 finding]` — `[regime-narrow]`
