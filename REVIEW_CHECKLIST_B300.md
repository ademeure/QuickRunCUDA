...
- [⚠] **B6** "DRAM write = 7.09 TB/s" — ⚠ NEEDS CLEAN RE-RUN. First attempt (justifications/B6_dram_write_clock_dep.md) reported 5.51/6.20/6.75/6.86 TB/s @ 1500/1700/1920/2032 MHz, but the agent's own methodology section flags **concurrent-process contamination** during the run ("found a stuck process from another concurrent four_fix worktree session... intermittent due to GPU-side race with concurrent agent"). User (2026-04-24) raised TWO additional concerns: (1) the "2032 MHz boost" reading is probably ncu-clamped to 1.92 GHz (so 6.86 vs 6.75 may be the same regime + noise), (2) catalog 7.09 likely correct under proper isolation. **User's clock-dep mechanism IS theoretically right** (at 1500 MHz, 148 SMs × 1500 × 32 B = 7.1 TB/s = right at HBM, so SM→L2 binds; at 1000 MHz only 4.7 TB/s = SM→L2 deeply binding), and the LOWER-clock measurements (5.51 / 6.20) are likely close to correct. The HIGH-clock measurements (6.75 / 6.86) need a clean re-run with strict serialization (pkill+rgc+verify-no-other-process) to validate vs catalog 7.09. Scheduled as last in the post-§20/§21/C5/B7B8D5 serialized chain. — `[ref: B300_PIPE_CATALOG.md:46, reviewed_errors L1132]`
