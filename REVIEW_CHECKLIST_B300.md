...
| **§21 tcgen05.mma sustained-load throttle** (RETEST 2026-04-24) | "100K iter cliff: cy/MMA jumps 128 → 305 → 394 at iters 30K → 50K → 100K; mechanism = tcgen05 dispatch governor / power tracking" | ❌ CLIFF DOES NOT REPRODUCE + I-cache hypothesis FALSIFIED. Across 5K → 100K iters at `#pragma unroll 1` / `unroll 8` / compiler-default, FP8 (M=128 N=128 K=32, 1 warp): **flat 64–67 cy/MMA**. FULL-UNROLL 30K (cubin = **9.1 MB**, 30 000 distinct UTCQMMA SASS at 96 B stride = 2.88 MB straight-line code, 90× I-cache size) measures only **70.7 cy/MMA = 5.6% slower than unroll=1** — refutes I-cache hypothesis AND original cliff. Catalog L7084 "Streaming throughput: 67 cy/MMA" is correct; the L7805–7811 cliff table appears to be a different regime / leftover-procs artifact. The "dispatch bubble / power tracking / governor" speculation should be deleted. See `21_tcgen05_throttle_ICACHE_DEEP.md`. |
...
**Genuinely measurement-blocked (deferred items, ~4):**
- ✅ **B6** DRAM write 7.09 TB/s — **RESOLVED 2026-04-23** via clock-locked sweep on GPU 0 (`justifications/B6_dram_write_clock_dep.md`): user's hypothesis CONFIRMED — SM→L2 write path scales with SM clock; canonical write peak = **6.86 TB/s @ 2032 MHz boost / 6.75 @ 1920 MHz lock**, not 7.09. See B6 line below.
...
These three remaining items (B7/B8/D5) still need a separate measurement campaign — outside this audit's scope.
...
- [⚠] **B6** "DRAM write = 7.09 TB/s" — ⚠ NEEDS CLEAN RE-RUN with **CORRECTED launch config**. First attempt (justifications/B6_dram_write_clock_dep.md) reported 5.51/6.20/6.75/6.86 TB/s @ 1500/1700/1920/2032 MHz but used **wrong block size (BS=512 instead of catalog-recipe BS=256)**, causing a 2-wave launch (4 CTAs/SM × 512 thr × 2 waves) instead of the catalog's single-wave (8 CTAs/SM × 256 thr × 1 wave). Both hit 2048 thr/SM max occupancy, but the 2-wave variant has tail-overhead + cache pollution between waves (3.7% `lts_lookup_hit` already shows partial L2 absorption). Catalog body explicitly specifies: *"DRAM write peak ≈ 7.0 TB/s with v8.u32 + 8 CTAs/SM × 256 threads = 2048 threads/SM"*. **User (2026-04-24) confirmed: catalog 7.09 IS correct (have seen even higher); B6's 6.86 is a launch-config artifact.** The clock-dep mechanism IS theoretically right (at 1500 MHz, 148 × 1500 × 32 B = 7.1 TB/s = right at HBM; at 1000 MHz only 4.7 TB/s deeply SM→L2-bound; at 64 SMs × boost similar) but B6's HIGH-clock numbers underestimate. Plus B6 had concurrent-process contamination (own doc admits: "stuck process from another concurrent four_fix worktree session"). **Retest scheduled**: BS=256, single-wave, strict serialization, WS≥4GB, with both wall-clock and ncu cross-check. — `[ref: B300_PIPE_CATALOG.md:46, reviewed_errors L1132]`
...
- [x] **C5** "FFMA2 + UNPACK u=1.67 (16% SMSP friction specific to F2FP)" — ✅ **CONFIRMED 2026-04-24** with refined mechanism (justifications/C5_ffma2_unpack_regport_DEEP.md). v52mirror replicates the asymmetry: F2FP+FFMA2 = u=3.37 vs PRMT+FFMA2 = u=3.98 (1.18× ratio = same 16% friction). **Mechanism is NOT raw RF read-port pressure** (PRMT actually has MORE reads/cycle). It is **operand-reuse cache disturbance**: F2FP UNPACK invalidates the SMSP `.reuse` cache slot used by FFMA2's broadcast multiplier, dropping `.reuse` rate from 99/128 (PRMT) to 79/128 (F2FP) → ~16% extra RF reads. **Regime caveat:** asymmetry only appears in clustered FFMA2-broadcast loops (where ptxas can stamp `.reuse`); in interleaved 1:1 with private multiplicands both reach u=2.74 (no `.reuse` to disturb). LOP3 holds `.reuse` only 50% AND has 3 srcs → 22% friction (u=2.14 in 1:1). — `[ref: B300_PIPE_CATALOG.md:478]`
...
- [x] **E9 (NEW)** §15 "with-return vs no-return same SASS, same rate" — ❌ FALSIFIED for global path (already top-13 fact C): no-return → REDG.E.ADD, with-return → ATOMG.E.ADD, **25× throughput delta**. TRUE for shared (ATOMS both ways). Catalog should split shared vs global rows. — `[ref: B300_PIPE_CATALOG.md:1024]`
- [x] **E10 (NEW)** §16 "atom.global.cas → ATOMG.E.CAS.STRONG.GPU" — ❌ FALSIFIED — already top-10 error #8. ACTUAL emit on this rig is **STRONG.SYS**. Catalog mis-named the scope. — `[ref: B300_PIPE_CATALOG.md:1064]`
- [x] **E11 (NEW)** `atomicAdd(addr, 1u)` no-return → ATOMS.POPC.INC.32 — ✅ CONFIRMED via SASS in justifications/22_atomic_smem_DEEP.md; already top-13 fact B. 2.5× speedup at warp-broadcast (24 vs 62 cy) via lane-combining popcount. Variable per-lane add stays at REDS.ADD. Catalog should add this optimization to §15. — `[NEW finding promoted to top-13]`
...
- [x] **G6** "mbarrier RTT = 54 cy single-thread count=1" — ⚠ PARTIALLY FALSIFIED via §0 cheat-sheet audit: catalog "mbarrier.arrive 8.1 cy" was arrive-only; actual RTT is **27 cy** for default `.shared.b64` modifier (top-10 error #7) and **123 cy** end-to-end RTT for the standard arrive→wait pattern (catalog 54 was arrive-only too). The 54-cy claim itself is misleading because end-to-end RTT depends on the arrive-then-wait sequence, not just the arrive cost. — `[ref: B300_PIPE_CATALOG.md:73]`
