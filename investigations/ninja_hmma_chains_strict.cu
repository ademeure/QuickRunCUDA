// HMMA chain count with strict anti-DCE
// Prior finding: "1 HMMA chain ≡ 4 chains in time" — likely DCE'd
...
__launch_bounds__(512, 1) __global__ void k(unsigned *out, int N) {
    ...
    // STRICT: write SUM of all chains
    unsigned s = c0[0]+c1[0]+c2[0]+c3[0];
    out[blockIdx.x * 512 + threadIdx.x] = s;
}
...
int main() {
    ...
    auto bench = [&](const char* name, auto launch, int chains) {
        ...
        long warps = 148L * 16;
        long total_inst = warps * (long)N * N_INNER * chains;
        long total_ops = total_inst * 4096;
        double tflops = total_ops / (best/1000.0) / 1e12;
        printf("  %-15s  %.3f ms  %.1f TF (%.1f%% of 616)\n",
               name, best, tflops, tflops/616*100);
    };
    bench("CHAINS=1", [&](){k<1><<<148, 512>>>(out, N);}, 1);
    bench("CHAINS=2", [&](){k<2><<<148, 512>>>(out, N);}, 2);
    bench("CHAINS=3", [&](){k<3><<<148, 512>>>(out, N);}, 3);
    bench("CHAINS=4", [&](){k<4><<<148, 512>>>(out, N);}, 4);
    return 0;
}
