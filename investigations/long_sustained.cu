// Test how long it takes to throttle on B300 with FP8 GEMM
...
int main() {
    ...
    printf("# B300 FP8 GEMM 8192³ throughput over time (each batch = 200 iters = ~50 ms)\n");
    printf("# %-6s %-12s %-15s %-10s\n", "batch", "elapsed_s", "TFLOPS", "vs_baseline");
    ...
    for (int batch = 0; batch < 60; batch++) {  // 60 batches = ~3 sec total
        ...
        if (batch < 5 || batch % 5 == 0) {
            char clk_buf[64];
            FILE *p = popen("nvidia-smi --query-gpu=power.draw,clocks.current.sm,temperature.gpu --format=csv,noheader -i 0", "r");
            fgets(clk_buf, sizeof(clk_buf), p); pclose(p);
            char *nl = strchr(clk_buf, '\n'); if (nl) *nl = 0;
            printf("  %-6d %-12.2f %-15.0f %-9.2fx %s\n",
                   batch, total_elapsed, tflops, tflops / baseline, clk_buf);
        }
    }
