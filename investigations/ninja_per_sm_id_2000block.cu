// Are the 8 "near" SMs the SAME for different addresses, or do they CHANGE?
// If same: dispatch order artifact (not real spatial locality)
// If different per address: real L2 subpartition mapping
#include <cuda_runtime.h>
#include <cstdio>
#include <cstdint>

__global__ void per_sm_lat(uint32_t *p, uint64_t *out, int max_sms) {
...
int main(int argc, char**argv) {
    ...
    per_sm_lat<<<2000, 32>>>((uint32_t*)((char*)d_p + offset), d_out, max_sms);
    ...
    long mid = (mn + mx) / 2;
    // Print which SM IDs are "near"
    printf("offset=%-12ld near SMs (cy<%ld):", offset, mid);
    for (int i = 0; i < n; i++) {
