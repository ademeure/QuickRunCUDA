// HBM read with high ILP unroll
#include <cuda_runtime.h>
#include <cstdio>

extern "C" __global__ void __launch_bounds__(512, 4) read_ilp(const int4 *data, int *out, int N, int iters) {
    ...
    int xor_sum = a.x ^ b.x ^ c.x ^ d.x ^ e.x ^ f.x ^ g.x ^ h.x;
    if (xor_sum == 0xdeadbeef) out[tid] = xor_sum;
}
...
int main() {
    ...
    // Warmup
    for (int i = 0; i < 3; i++) read_ilp<<<blocks, threads>>>(d_data, d_out, N, 1);
    cudaDeviceSynchronize();

    float best = 1e30f;
    for (int trial = 0; trial < 5; trial++) {
        cudaEventRecord(e0);
        read_ilp<<<blocks, threads>>>(d_data, d_out, N, iters);
        cudaEventRecord(e1);
    ...
    printf("# B300 HBM read with 8-way ILP, 512 thr/blk, 4 GB working set\n");
    printf("  Time: %.2f ms\n", best);
