// Ninja absmax v2: block-level reduce to avoid atomic contention
...
// Block does multiple per-warp loads, then block-level reduce, ONE atomic per block
__launch_bounds__(256, 4) __global__ void absmax_blkreduce(
    const __nv_bfloat16 *x, float *out, int N)
{
    __shared__ float smem[8];  // one per warp
    ...
    float local = 0.f;
    // Each thread reads multiple uint4s (8 BF16 = 16 B per uint4)
    int warp_in_grid = blockIdx.x * 8 + warp;
    int total_warps = gridDim.x * 8;
    // Each warp covers some chunk of 1024 B at a time
    for (int wbase = warp_in_grid * 512; wbase < N; wbase += total_warps * 512) {
        // 32 lanes × 16 BF16 each = 512 BF16 per warp = 1024 B
        int my_base = wbase + lane * 16;
        if (my_base >= N) break;
        uint4 v0 = *(uint4*)&x[my_base];
        uint4 v1 = *(uint4*)&x[my_base + 8];
    ...
    __syncthreads();
    // block reduce: only lane 0 of warp 0
    if (warp == 0) {
...
int main() {
    ...
    auto bench = [&](int blocks, const char* label) {
        cudaMemset(d_out, 0, sizeof(float));
        for (int i = 0; i < 5; i++) {
            cudaMemset(d_out, 0, sizeof(float));
            absmax_blkreduce<<<blocks, 256>>>(d_x, d_out, N);
        }
        ...
        printf("  blocks=%6d: %.4f ms = %.1f GB/s = %.2f%% spec (result=%.4f)\n",
               blocks, best, gbs, gbs/7672*100, r);
    };

    // Try multiple block counts
    for (int b : {148, 148*2, 148*4, 148*8, 148*16, 148*32, 148*64, 148*128, 148*256}) {
        bench(b, "");
    }
