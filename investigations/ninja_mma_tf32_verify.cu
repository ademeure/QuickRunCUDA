// TF32 mma.sync m16n8k8 — verify catalog 289.6 TF / 288 spec
...
__launch_bounds__(512, 1) __global__ void k(unsigned *out, int N) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        for (int j = 0; j < N_INNER; j++) {
            mma_tf32(c0, a0, b0, c0);
            mma_tf32(c1, a1, b1, c1);
            mma_tf32(c2, a2, b2, c2);
            mma_tf32(c3, a3, b3, c3);
        }
    }
...
int main() {
    ...
    long warps = 148L * 16, ops_per_inst = 2L * 16 * 8 * 8;  // m16n8k8 → 2048 ops
    long total_ops = warps * (long)N * N_INNER * 4 * ops_per_inst;
    double tflops = total_ops / (best/1000.0) / 1e12;
    printf("# TF32 mma.sync m16n8k8 f32.tf32.tf32.f32\n");
    printf("  best=%.3f ms  %.1f TFLOPS  (catalog: 288, this/catalog = %.0f%%)\n",
